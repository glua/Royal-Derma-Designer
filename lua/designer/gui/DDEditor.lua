...
function PANEL:Init()
	...
	self.m_colText = Color(255,0,0,255)
	self:SetCursorColor(255,255,255,255)
...
function PANEL:Paint( w, h )
	...
	local rows = string.Explode( "\n", self:GetValue() )
		for k,v in ipairs( rows ) do
			local word = string.Explode( " ", v )
			for a,b in ipairs( word ) do
				if( b == "a" ) then
				--self:DrawTextEntryText( Color(255,0,0,255), self.m_colHighlight, self.m_colCursor ) DermaDefault
				draw.SimpleText( b, "DermaDefault",  x,  y, Color(255,0,0,255))
				else
...
derma.DefineControl( "DDEditor", "A standard Button", PANEL, "TextEntry" )
