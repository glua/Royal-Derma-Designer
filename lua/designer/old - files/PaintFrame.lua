...
frame:SetDragable( false )
 ...
 local test = vgui.Create("DPanel" )
	test:SetPos(0,0)
	test:SetSize(sheet:GetWide(),sheet:GetTall())
	...
	sheet:AddSheet( "test", test, "icon16/cross.png" )

	 local test2 = vgui.Create("DPanel" )
	test2:SetPos(0,0)
	test2:SetSize(sheet:GetWide(),sheet:GetTall())
	function test2:Paint(w,h)

	surface.SetDrawColor(255,0,0,150)
	surface.DrawRect(0,0,w,h)

	end
		sheet:AddSheet( "test2", test2, "icon16/cross.png" )
	...
	e:AddButton("DD/gui/mouse.png",function() designer:SetModus("mouse") end )
	...
	e:AddButton("DD/gui/poly_4.png",function() designer:SetModus("4poly") end )
		...
		if( #EPanel:GetChildren() > 0 ) then
			for k,v in ipairs( EPanel:GetChildren() ) do
				if( v:GetSelect() ) then
					...
					designer.layer[v:GetID()].col = col
